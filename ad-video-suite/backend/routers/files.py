...
async def serve_file(path: str = Query(...), download: bool = False):
    p = _safe_path(path)
    if not p.exists():
        raise HTTPException(status_code=404, detail=f"File not found: {p}")
    if not p.is_file():
    ...
    disposition = f"attachment; filename={p.name}" if download else "inline"
    return FileResponse(str(p), media_type=media_type, headers={"Content-Disposition": disposition})
